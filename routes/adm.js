...
router.get('/board/write/:board_id', checking, async function(req, res, next) {
    ...
    if (idx) {
        var sql = `SELECT * FROM BOARD_tbl WHERE idx = ?`;
        var params = [idx];
        var arr = await utils.queryResult(sql, params);
        row = arr[0];
    }
    ...
    console.log(row);
    res.render(`./adm/${board_id}_write.html`, {
        myinfo: req.session,
        return_url,
        row,
    });
...
router.get('/grade/write', checking, async function(req, res, next) {
    const idx = req.query.idx;
    const return_url = req.query.return_url;
    ...
    if (idx) {
        var sql = `SELECT * FROM GRADE_tbl WHERE idx = ?`;
        var params = [idx];
        var arr = await utils.queryResult(sql, params);
        row = arr[0];
    }
    ...
    res.render(`./adm/grade_write.html`, {
        myinfo: req.session,
        return_url,
        row,
    });
...
router.get('/manager/write', checking, async function(req, res, next) {
    const idx = req.query.idx;
    const return_url = req.query.return_url;
    ...
    //권한 불러오기!
    sql = ` SELECT * FROM GRADE_tbl ORDER BY level1 ASC `;
    arr = await utils.queryResult(sql, []);
    row.grades = arr;
    //
    
    res.render(`./adm/manager_write.html`, {
        myinfo: req.session,
        return_url,
        row,
    });
...
router.get('/user/write', checking, async function(req, res, next) {
    const idx = req.query.idx;
    const return_url = req.query.return_url;
    ...
    res.render(`./adm/user_write.html`, {
        myinfo: req.session,
        return_url,
        row,
    });
